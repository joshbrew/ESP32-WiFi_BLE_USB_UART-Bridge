...
This is based on my prior work with the ESP32 but optimizes a ton, leaving about 33KB of program memory available when using both WiFi and BLE stacks and they can be run simultaneously using the dual core system. This is a shell for adding on sensors and controls when we want to use the ESP32's radios, and has extensive controls e.g. for power usage and NVS-saved radio configurations (which radios to enable, credentials, names, etc). This is a full suite so you only need to add your own sensor and controller programs on.
