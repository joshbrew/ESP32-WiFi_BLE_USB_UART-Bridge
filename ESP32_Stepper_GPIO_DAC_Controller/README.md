# ESP32 Stepper + DAC + GPIO RTOS Controller

Firmware version: `2026-07-27-v5.15.44-dac1-gpio26`

This is the hardware-specific version of the ESP32 modular command and transport controller. It combines a nonblocking FreeRTOS stepper driver, one ESP32 hardware DAC output, one digital GPIO output, USB serial, Wi-Fi HTTP, BLE UART, optional Classic Bluetooth SPP, optional auxiliary UART, a browser console, persistent configuration, radio-profile rollback, status indicators, self-test, and validated HTTP OTA.

The stepper, DAC, and digital-output implementation is isolated under `src/addons/stepper_dac/`. The companion transport-only repository uses the same radio, web, OTA, command, and indicator framework without owning the motor, DAC, or GPIO26 output pins.
...
- [DAC and GPIO output behavior](#dac-and-gpio-output-behavior)
...
- Controls DAC1 on GPIO25 as an adjustable analog output.
- Controls GPIO26 as a normal push-pull digital output with LOW and HIGH states.
- Supports timed tests, boot-enable state, automatic safety timeout, and NVS persistence for both outputs.
- Exposes the same command queue through USB serial, Wi-Fi, BLE, optional SPP, and optional auxiliary UART.
- Hosts a compact browser console with stepper shortcuts, hold-to-run DAC and GPIO controls, live state, event logs, radio controls, self-test, and firmware OTA.
...
- Includes a persistent multi-boot self-test that snapshots and restores radio, system, stepper, DAC, and GPIO-output settings.
...
One native ESP32 DAC output on GPIO25
Digital push-pull output on GPIO26
```
...
```text
GPIO25 -> adjustable DAC1 analog signal
GPIO26 -> digital LOW/HIGH control signal
ESP32 GND -> receiving circuit ground
```

DAC1 is an 8-bit native ESP32 DAC and produces a voltage between ground and its calibrated reference, nominally around 3.3 V. Treat it as a signal source for a high-impedance input.

GPIO26 is now configured as a normal push-pull digital output. `GPIO26:ON` produces a logic HIGH near the ESP32 3.3 V rail and `GPIO26:OFF` produces LOW. It is intended to control an external switch such as a CD74HCT4066 when the actual switched signal is 4 V. Do not feed the 4 V signal into GPIO26 and do not use either output as a power source.
...
| Digital output | 26 | `DacController` |
...
This rebuilds the embedded gzip portal and standalone console, checks JavaScript syntax and HTML bindings, validates source layout, checks OTA markers, and audits configured pin ownership. `web/app.min.js` is hash-checked against `web/app.js`; install a local `terser` package before rebuilding after JavaScript changes.
...
Hold-to-run DAC1 and GPIO26 buttons
...
The hold-to-run output buttons send `DAC1:ON` or `GPIO26:ON` while held and send the matching `OFF` command on release, pointer cancellation, lost pointer capture, window blur, page hide, and tab hiding.
...
`StopAll` clears the unified command queue, queues an urgent motor coil release, and releases DAC1 and drives GPIO26 LOW.
...
## DAC and GPIO output behavior

The addon owns two different output types:

```text
DAC1   -> GPIO25, adjustable 8-bit analog output
GPIO26 -> GPIO26, push-pull digital LOW/HIGH output
```

Default state:

```text
DAC reference calibration: 3300 mV
DAC1 target: 500 mV
DAC1 boot output: off
GPIO26 boot output: LOW
Safety timeout: unlimited, value 0
```

`DACRefMV` calibrates only DAC1's code conversion against its measured full-scale output. It does not change GPIO26 and it does not create a higher supply rail.
...
`DAC1:ON` drives the configured analog target. `GPIO26:ON` drives a digital HIGH near 3.3 V. The matching `OFF` command releases DAC1 or drives GPIO26 LOW.

`DACAll` and `OutputAll` affect both outputs. `StopAll` also releases DAC1 and drives GPIO26 LOW.
...
Changing `DAC1:MV` updates the target. An already enabled DAC1 updates immediately. A disabled DAC1 remains disabled until turned on.

GPIO26 is digital-only. `GPIO26:MV:...` and `DAC2:MV:...` return an error so the firmware cannot silently pretend a digital output is an adjustable DAC.
...
`DAC1:TEST3S` drives 500 mV for 3000 ms, then releases DAC1. `GPIO26:TEST3S` drives HIGH for 3000 ms, then returns LOW. `DACTest3S` runs both tests together.
...
The `DAC2:` prefix remains accepted for the GPIO26 timeout commands. A value of `0` means unlimited. Setting a timeout while an output is active restarts its timeout from that moment.

Examples:

```text
DAC1:TIMEOUTMS:5000
GPIO26:TIMEOUTSEC:30
```
...
The `DAC2:` prefix remains accepted as a compatibility alias for GPIO26 boot-state commands. `GPIO26:BOOT:ON` means the pin is driven HIGH when saved output settings are loaded at boot.

Use `DACSave` to persist the DAC1 reference and target plus both outputs' boot states and safety timeouts. `DACLoad` reloads the saved settings and applies the saved boot states immediately.

`DACDefaults` restores volatile defaults, releases DAC1, and drives GPIO26 LOW. `DACErase` clears the saved output namespace while leaving current volatile settings unchanged.
...
GPIO26:ON
GPIO26:OFF
DAC2:ON                         compatibility alias
DAC2:OFF                        compatibility alias
DACAll:ON
DACAll:OFF
OutputAll:ON
OutputAll:OFF
...
`DAC2:` remains an alias for GPIO26 except that `DAC2:MV:...` is rejected because GPIO26 is digital-only.
...
### 7. Hold an output from the browser
...
### 7a. Drive the external-switch control
...
When connected to a CD74HCT4066 control input, `GPIO26:ON` closes the selected analog switch channel and allows the external 4 V signal to pass through the switch. The ESP32 pin itself remains at 3.3 V logic.
...
This clears queued general commands, prioritizes motor coil release, and releases DAC1 and drives GPIO26 LOW.
...
│   │       ├── DacController.*                 DAC/GPIO output state, timers, NVS, hardware writes
...
The Arduino sketch stays small. Motor behavior belongs in `StepperController`, DAC and digital-output behavior belongs in `DacController`, and shared radio/web/transport code does not directly touch addon pins.
...
src/addons/stepper_dac/DacController.cpp      DAC/GPIO command parser and safety timers
...
DAC1 cannot produce 4 V. GPIO26 is now the 3.3 V logic control output for an external analog switch such as the CD74HCT4066. Route the actual 4 V signal through the switch terminals, never through the ESP32 GPIO.

### An output stays active too long
...
Use `StopAll` to release DAC1, drive GPIO26 LOW, and release the motor coils immediately.
...
- Configure output safety timeouts for outputs that can affect external hardware.
- GPIO25 remains limited to the ESP32 DAC rail. GPIO26 is 3.3 V digital logic and must only control a logic input such as the CD74HCT4066 enable pin.
