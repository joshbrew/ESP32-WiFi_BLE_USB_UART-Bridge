...
def minify_javascript(text: str) -> str:
    ...
    minified = WEB / "app.min.js"
    local_bin = ROOT / "node_modules" / ".bin" / ("terser.cmd" if sys.platform == "win32" else "terser")
    terser = str(local_bin) if local_bin.exists() else shutil.which("terser")
    ...
    if terser:
        with tempfile.TemporaryDirectory() as directory:
            ...
            subprocess.run(
                [terser, str(source), "--compress", "passes=3", "--mangle", "--comments", "false", "-o", str(output)],
                cwd=ROOT,
                check=True,
            )
            compact = output.read_text(encoding="utf-8").strip()
        minified.write_text(f"/* source-sha256:{source_hash} */\n{compact}\n", encoding="utf-8")
        return compact

    if not minified.exists():
        fail("missing web/app.min.js; install terser locally after changing web/app.js")
    cached = minified.read_text(encoding="utf-8")
    marker = re.match(r"/\* source-sha256:([0-9a-f]{64}) \*/\s*", cached)
    if marker is None or marker.group(1) != source_hash:
        fail("web/app.min.js is stale; install terser locally and rerun tools/project_tool.py all")
    return cached[marker.end():].strip()
...
def build_web_assets() -> None:
    ...
    lines.extend(["}  // namespace WebAssets", "", "#endif  // ESP32_STEPPER_DUAL_DAC_WEBASSETS_H", ""])
...
(() => {
  "use strict";
  const storageKey = "esp32-stepper-device-url";
...
'''
    ...
    output = output.replace(
        ...
        "<title>ESP32 Stepper + DAC + GPIO Standalone Console</title>",
    )
...
def check_layout() -> None:
    sketch = ROOT / "ESP32_Stepper_Dual_DAC_RTOS_Controller.ino"
    ...
    for folder in [
        "src/addons/stepper_dac", "src/config", "src/core", "src/hardware",
...
def check_comment_terminators() -> None:
    suspicious: list[str] = []
    for path in [ROOT / "ESP32_Stepper_Dual_DAC_RTOS_Controller.ino", *sorted(SRC.rglob("*.h")), *sorted(SRC.rglob("*.cpp"))]:
        text = path.read_text(encoding="utf-8")
...
def check_markers_and_architecture() -> None:
    ...
    required: dict[str, list[str]] = {
        "ESP32_Stepper_Dual_DAC_RTOS_Controller.ino": [
            '#include "src/config/AppConfig.h"', "APP_STEPPER_DAC_ADDON_ENABLED",
            "TransportBridge transportBridge", "StepperDacAddon deviceAddon", "DeviceAddon deviceAddon",
            "deviceAddon.begin();", "deviceAddon.queueStartupTests(dispatcher);",
            "CommandRouter::webStateThunk",
        ],
        "src/config/AppConfig.h": [
            'FIRMWARE_VERSION = "2026-07-27-v5.15.44-dac1-gpio26"',
            "#define APP_ADDON_STEPPER_DAC", "#define APP_ENABLE_WIFI",
            "#define APP_ENABLE_BLE", "#define APP_ENABLE_STATUS_INDICATORS",
            "#define APP_ENABLE_STATUS_LED_BOOT_TEST", "HTTP_MAX_ACTIVE_REQUESTS = 2",
            "HTTP_REMAINING_FREE_HEAP_BYTES = 9000", "HTTP_MIN_LARGEST_BLOCK_BYTES = 6000",
            "HTTP_CLIENT_RX_TIMEOUT_SECONDS", "HTTP_CLIENT_ACK_TIMEOUT_MS",
            "BLE_TX_MIN_FREE_HEAP_BYTES", "BLE_NOTIFY_SUBSCRIPTION_SETTLE_MS",
        ],
        "src/addons/DeviceAddon.h": ["class DeviceAddon", "virtual bool handleCommand", "virtual void service"],
        "src/addons/stepper_dac/StepperDacAddon.cpp": [
            'return "stepper-dac";', "stepper_.begin();", "dac_.begin();",
            "dac_.handleCommand", "stepper_.handleCommand",
        ],
        "src/core/AppTypes.h": [
        ...
        "src/transports/TransportBridge.cpp": [
            '"SendBLE"', '"SendWiFi"', '"SendUSB"', 'parsePayload(command, "Send", payload)',
            "active,\n    true", "isBleOutputReady()", "hasRecentClient()",
        ],
        ...
        "src/web/WebPortal.cpp": [
            ...
            "AsyncBasicResponse owns its String content",
            "setRxTimeout", "setAckTimeout", "request->abort()",
            "HTTP_MAX_ACTIVE_REQUESTS", "HTTP_MAX_ACTIVE_RESERVED_BYTES",
            "inspectOrReserveRequest", "WebAssets::PORTAL_HTML_GZIP",
            "Update.begin(otaExpectedBytes_, U_FLASH)", "Update.end(false)",
            "X-Firmware-Size", "OTA_IMAGE_MAGIC", "otaRequest_ == request",
        ],
        "src/radio/RadioManager.cpp": [
            "BootRadioMode::WIFI_BLE_P", "ModeWiFiBLEP", "isPersistentWifiBleProfile",
            "webPortal_.discardClientMetadata()",
        ],
        "web/index.html": [
            'data-hold-on="DAC1:ON"', 'data-hold-off="DAC1:OFF"',
            'data-hold-on="GPIO26:ON"', 'data-hold-off="GPIO26:OFF"',
        ],
        "web/app.js": [
            "httpTail:Promise.resolve()", "function postHttp", "state.httpTail.then",
            '"Content-Type":"application/octet-stream"', "body:clean",
            "releaseAllDacs", 'runCommand(button.dataset.holdOff,true)',
            'if(event.q)', '"ModeWiFiBLEP"', "new FormData()",
            'form.append("firmware",file,file.name)', '"X-Firmware-Size"',
            "prepareHttpForOta", "magic!==0xe9",
        ],
        "web/app.min.js": [
            'card("GPIO 26",m())', '`HIGH${t.oneShot?" test":""}`',
        ],
    }
    ...
    if missing:
        fail("v5.15.44 architecture validation failed: " + "; ".join(missing))
    print("OK: modular addon layout, source-scoped event routing, BLE lifecycle guard, and bounded HTTP admission are present")
...
def check_pin_map() -> None:
    ...
    expected = {
        "PIN_STEPPER_IN1": 18, "PIN_STEPPER_IN2": 19,
        "PIN_STEPPER_IN3": 27, "PIN_STEPPER_IN4": 14,
        "PIN_STATUS_CONNECTION": 23, "PIN_STATUS_ACTIVITY": 5,
        "PIN_DAC_1": 25, "PIN_DIGITAL_OUTPUT": 26,
    }
    found: dict[str, int] = {}
    ...
    if len(set(found.values())) != len(found):
        fail("pin map contains duplicate GPIO assignments")
    print("OK: stepper, DAC, digital output, and indicator pin map is conflict-free")
...
def validate() -> None:
    ...
    if node:
        ...
        run([node, "--check", str(WEB / "app.min.js")])
    else:
