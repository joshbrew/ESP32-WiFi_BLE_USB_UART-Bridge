/*
  DRONE GEL DISPENSER + MODULAR COMMAND CONTROLLER

  BEGIN HERE: open the QuickConfig.h tab to choose the hardware profile, pinout,
  pulse limits, optional interlock, status LEDs, and local Wi-Fi name. Advanced
  build switches and internal limits remain in src/config/AppConfig.h.
*/
...
#elif APP_STEPPER_DAC_ADDON_ENABLED
#include "src/addons/stepper_dac/DacController.h"
#include "src/addons/stepper_dac/StepperController.h"
#include "src/addons/stepper_dac/StepperDacAddon.h"
#endif
...
#if APP_DRONE_DISPENSER_ADDON_ENABLED
DispenserAddon deviceAddon(events);
#elif APP_STEPPER_DAC_ADDON_ENABLED
StepperController stepper(events);
DacController dac(events);
StepperDacAddon deviceAddon(events, stepper, dac);
#else
DeviceAddon deviceAddon;
#endif

RoutineEngine routines(events, deviceAddon);
...
void setup() {
  ...
  // Initialize selected hardware before NVS tests, indicator animations, or
  // radio startup. The default dispenser profile establishes its safe output
  // here; the optional bench profile applies its explicitly saved settings.
  Serial.printf("[BOOT][ADDON] early safe initialization selected=%s\n", deviceAddon.name());
  Serial.flush();
  deviceAddon.begin();
