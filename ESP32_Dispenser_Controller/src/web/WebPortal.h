...
#error Remove old/duplicate AsyncTCP libraries and install "Async TCP" by ESP32Async, version 3.4.10 or newer.
#endif
...
class WebPortal {
 ...
 private:
  ...
  bool prepared_;
  bool running_;
  std::atomic<uint32_t> requestCount_;
  std::atomic<uint32_t> lastRequestAtMs_;
  ...
  std::atomic<uint8_t> activeRequestCount_;
  std::atomic<size_t> activeReservedBytes_;
  std::atomic<uint32_t> pressureRejectCount_;
#if APP_HTTP_OTA_ENABLED
  bool otaUploadActive_;
  bool otaUploadSuccess_;
  bool otaImageValidated_;
  std::atomic<bool> otaRebootPending_;
  ...
  std::atomic<bool> otaSafetyStopRequested_;
#endif
};
#else
class WebPortal {
