...
#if APP_HTTP_OTA_ENABLED
  if (
    otaRebootPending_.load(std::memory_order_acquire) &&
    static_cast<int32_t>(millis() - otaRebootAtMs_) >= 0
  ) {
...
uint32_t WebPortal::lastRequestAtMs() const {
  return lastRequestAtMs_.load(std::memory_order_relaxed);
}

bool WebPortal::hasRecentClient(uint32_t maxAgeMs) const {
  const uint32_t last = lastRequestAtMs_.load(std::memory_order_relaxed);
  return running_ && requestCount_.load(std::memory_order_relaxed) > 0 && last != 0 &&
    static_cast<uint32_t>(millis() - last) <= maxAgeMs;
}

void WebPortal::discardClientMetadata() {
  lastRequestAtMs_.store(0, std::memory_order_relaxed);
  if (sharedMutex_ == nullptr) {
    return;
  }
...
bool WebPortal::consumeSafetyStopRequest() {
#if APP_HTTP_OTA_ENABLED
  return otaSafetyStopRequested_.exchange(false, std::memory_order_acq_rel);
#else
  return false;
#endif
}
...
bool WebPortal::admitRequest(
  ...
  if (
    activeRequestCount_.load(std::memory_order_relaxed) < AppConfig::HTTP_MAX_ACTIVE_REQUESTS &&
    activeReservedBytes_.load(std::memory_order_relaxed) + reservedBytes <= AppConfig::HTTP_MAX_ACTIVE_RESERVED_BYTES
  ) {
    for (ActiveRequestRecord &record : activeRequests_) {
      ...
      activeRequestCount_.fetch_add(1, std::memory_order_relaxed);
      activeReservedBytes_.fetch_add(reservedBytes, std::memory_order_relaxed);
      admitted = true;
      break;
    }
  }
...
bool WebPortal::takeCommandBody(
  ...
  for (ActiveRequestRecord &record : activeRequests_) {
    ...
    const size_t activeReserved = activeReservedBytes_.load(std::memory_order_relaxed);
    if (activeReserved >= bodyCapacity) {
      activeReservedBytes_.store(activeReserved - bodyCapacity, std::memory_order_relaxed);
    } else {
      activeReservedBytes_.store(0, std::memory_order_relaxed);
    }
    break;
  }
  portEXIT_CRITICAL(&activeRequestMux_);
...
void WebPortal::releaseRequest(AsyncWebServerRequest *request) {
...
#endif
    const uint8_t activeCount = activeRequestCount_.load(std::memory_order_relaxed);
    if (activeCount > 0) {
      activeRequestCount_.store(static_cast<uint8_t>(activeCount - 1), std::memory_order_relaxed);
    }
    const size_t activeReserved = activeReservedBytes_.load(std::memory_order_relaxed);
    if (activeReserved >= record.reservedBytes) {
      activeReservedBytes_.store(activeReserved - record.reservedBytes, std::memory_order_relaxed);
    } else {
      activeReservedBytes_.store(0, std::memory_order_relaxed);
    }
  ...
  if (interruptedOta) {
    ...
    otaUploadSuccess_ = false;
    otaImageValidated_ = false;
    otaRebootPending_.store(false, std::memory_order_release);
    otaRequest_ = nullptr;
  ...
  if (startedAtMs != 0) {
    Serial.printf(
      ...
      static_cast<unsigned>(activeRequestCount_.load(std::memory_order_relaxed)),
      static_cast<unsigned>(activeReservedBytes_.load(std::memory_order_relaxed)),
...
void WebPortal::rejectForPressure(AsyncWebServerRequest *request) {
  pressureRejectCount_.fetch_add(1, std::memory_order_relaxed);
  if (request == nullptr || request->client() == nullptr) {
    return;
  }
#if APP_VERBOSE_COEX_HTTP_DIAGNOSTICS
  Serial.printf(
    ...
    static_cast<unsigned>(ESP.getFreeHeap()),
    static_cast<unsigned>(ESP.getMaxAllocHeap()),
    static_cast<unsigned>(pressureRejectCount_.load(std::memory_order_relaxed))
  );
  Serial.flush();
#endif
...
void WebPortal::noteRequest(
  ...
  lastRequestAtMs_.store(millis(), std::memory_order_relaxed);
  ...
  Serial.printf(
    ...
    static_cast<unsigned>(requestCount_.load(std::memory_order_relaxed))
  );
  Serial.flush();
#else
  (void)request;
...
void WebPortal::handleOtaUpload(
  ...
  if (index == 0) {
    ...
    // The upload callback may run on AsyncTCP's task. Raise a one-shot atomic
    // request so the normal control task performs the definitive hardware
    // shutdown, and also enqueue StopAll as a command barrier for defense in
    // depth before flash writing begins.
    otaSafetyStopRequested_.store(true, std::memory_order_release);
...
void WebPortal::handleOtaComplete(AsyncWebServerRequest *request) {
  ...
  if (success) {
    ...
    otaRebootPending_.store(true, std::memory_order_release);
...
void WebPortal::discardClientMetadata() {}
bool WebPortal::consumeSafetyStopRequest() { return false; }
#endif
