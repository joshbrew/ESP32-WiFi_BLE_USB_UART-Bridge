...
void StepperDacAddon::appendStateJson(String &json, bool compact) const {
  json += ",\"addon\":{\"name\":\"stepper-dac\",\"active\":true,\"dispenser\":false,\"stepper\":true,\"dac\":true}";
...
void StepperDacAddon::publishHelp(
  ...
  events.publish(EventLevel::STATUS, "Outputs: DAC1 MV/ON/OFF/BOOT/TIMEOUT/TEST3S; GPIO26 ON/OFF/BOOT/TIMEOUT/TEST3S; DAC2 aliases GPIO26 except MV", source, requestId);
  events.publish(EventLevel::STATUS, "Output config: DACStatus DACRefMV DACSave DACLoad DACDefaults DACErase", source, requestId);
}
...
String StepperDacAddon::runtimeRestoreCommand(uint16_t index) const {
  switch (index) {
    ...
    case 15: return String("gpio26:boot:") + (snapshot_.dacBootEnabled[1] ? "on" : "off");
    case 16: return "gpio26:timeoutms:" + String(snapshot_.dacTimeoutMs[1]);
    case 17: return "gpio26:off";
