...
class DeviceAddon {
 public:
  ...
  // Called before a saved routine starts. Safety-oriented addons can require
  // an explicit arm or interlock without teaching the routine engine about a
  // particular piece of hardware.
  virtual bool canStartRoutine(String &reason) const {
    reason = "ready";
    return true;
  }
  ...
  // routine engine's own INTERNAL commands.
  virtual bool blocksExternalCommandDuringRoutine(const String &command) const {
    (void)command;
    return false;
  }
  ...
  virtual void publishHelp(
    ...
    events.publish(
      EventLevel::STATUS,
      "Addon: none selected; choose a hardware profile in QuickConfig.h",
      source,
      requestId
    );
  }
