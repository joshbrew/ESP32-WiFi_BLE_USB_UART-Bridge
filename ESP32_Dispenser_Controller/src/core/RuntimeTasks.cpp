...
bool RuntimeTasks::begin() {
  ...
  events_.publish(
    EventLevel::STATUS,
    "network, command, routine, and addon service ready stack=" +
...
// bounded so Wi-Fi, command output, and DAC fallback timing stay responsive.
// WebPortal::service() refreshes a cached state snapshot only; AsyncTCP owns sockets.
void RuntimeTasks::serviceOnce() {
  if (webPortal_.consumeSafetyStopRequest()) {
    router_.forceHardwareSafe("OTA upload requested");
  }

  const bool radioRebootPending = radios_.managedRebootPending();
  if (radioRebootPending && !radioSafetyApplied_) {
    radioSafetyApplied_ = router_.forceHardwareSafe("radio profile reboot pending");
  } else if (!radioRebootPending) {
    radioSafetyApplied_ = false;
  }
