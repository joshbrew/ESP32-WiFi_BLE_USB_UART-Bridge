...
CommandRouter::CommandRouter(
  ...
  StatusIndicators &indicators
    ...
    dispatcher_(nullptr),
    selfTest_(nullptr),
    productionMode_(AppConfig::DEFAULT_PRODUCTION_MODE),
...
void CommandRouter::begin() {
  ...
  if (preferences.begin(SYSTEM_PREFERENCES_NAMESPACE, true)) {
    productionMode_ = preferences.getBool(
      PRODUCTION_MODE_KEY,
      AppConfig::DEFAULT_PRODUCTION_MODE
    );
    preferences.end();
  }
...
void CommandRouter::attachDispatcher(CommandDispatcher *dispatcher) {
  ...
  routines_.configureSubmitter(CommandDispatcher::commandThunk, dispatcher);
}
...
void CommandRouter::submit(
  ...
  if (emergencyStop) {
    ...
    const bool addonStopped = routines_.stop(source, requestId, "StopAll", false);
    if (addonStopped) {
  ...
  if (command.equalsIgnoreCase("Reboot")) {
    ...
    const bool addonStopped = routines_.stop(source, requestId, "controller reboot", false);
    publish(
  ...
  if (
    ...
    error(source, requestId, "saved routine owns the payload; send RoutineStop or StopAll first");
    return;
  }
...
bool CommandRouter::forceHardwareSafe(const String &reason) {
  ...
  const bool safe = routines_.stop(
    CommandSource::INTERNAL,
    "system-safety",
    reason,
    false
  ...
  publish(
    safe ? EventLevel::WARNING : EventLevel::ERROR,
    CommandSource::INTERNAL,
    "system-safety",
    String(safe ? "[SAFE] " : "[FAULT] ") + reason +
      (safe ? "; pending commands cleared and hardware stopped" : "; hardware stop was incomplete")
  );
  return safe;
}
...
void CommandRouter::publishHelp(CommandSource source, const String &requestId) {
  ...
  routines_.publishHelp(source, requestId);
}
...
void CommandRouter::publishStatus(CommandSource source, const String &requestId) {
  ...
  publishChunked(EventLevel::STATUS, source, requestId, "[STATUS] routines", routines_.statusText());
...
void CommandRouter::publishConfig(CommandSource source, const String &requestId) {
  ...
  publishChunked(EventLevel::STATUS, source, requestId, "[CONFIG] routines", routines_.statusText());
