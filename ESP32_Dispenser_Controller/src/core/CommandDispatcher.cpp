...
void CommandDispatcher::service(uint8_t budget) {
  ...
  while (processed < budget && xQueuePeek(queue_, &command, 0) == pdTRUE) {
    ...
    // when a subsystem's private queue is temporarily full. Stop/CoilsOff stay
    ...
    const bool emergencyCommand = isEmergencyCommand(line);
    if (emergencyPending_.load(std::memory_order_acquire) && !emergencyCommand) {
      // An emergency producer may have raised the latch after this normal
      // command was received. Drop it so no activation can slip between the
      // stop request and the StopAll queue barrier.
      continue;
    }
    router_.submit(command.source, line, String(command.requestId));
    if (emergencyCommand) {
      emergencyPending_.store(false, std::memory_order_release);
      processed++;
      break;
    }
    processed++;
...
void CommandDispatcher::clearPending(bool clearEmergencyLatch) {
  if (queue_ == nullptr) {
    return;
  }
...
bool CommandDispatcher::submit(
  ...
  if (isEmergencyCommand(line)) {
    return submitEmergency(source, line, requestId);
  }
...
bool CommandDispatcher::submitBatch(
  ...
  if (lineCount == 1 && isEmergencyCommand(normalized)) {
    const bool accepted = submitEmergency(source, normalized, requestId);
    acceptedLines = accepted ? 1 : 0;
    return accepted;
  }
...
bool CommandDispatcher::submitEmergency(
  CommandSource source,
  const String &line,
  const String &requestId
) {
  ...
  // Raise the latch before waiting for the admission mutex. The control task
  // will discard a normal command it may already have received, while the
  // queue reset makes StopAll the next command that can execute. Routing stays
  // on the single control task instead of racing an AsyncTCP/BLE callback.
  emergencyPending_.store(true, std::memory_order_release);
  if (!lockSubmitter(portMAX_DELAY)) {
    emergencyPending_.store(false, std::memory_order_release);
    reject(source, requestId, "could not acquire emergency command barrier");
    return false;
  }
  emergencyPending_.store(true, std::memory_order_release);
  const UBaseType_t discarded = uxQueueMessagesWaiting(queue_);
  xQueueReset(queue_);
  const BaseType_t queued = xQueueSendToFront(queue_, &command, 0);
  unlockSubmitter();

  if (queued != pdTRUE) {
    emergencyPending_.store(false, std::memory_order_release);
    reject(source, requestId, "could not queue emergency StopAll barrier");
    return false;
  }
  ...
  events_.publish(
    EventLevel::WARNING,
    "[QUEUED] emergency StopAll barrier accepted; discarded=" + String(discarded),
    source,
    requestId
  );
  return true;
